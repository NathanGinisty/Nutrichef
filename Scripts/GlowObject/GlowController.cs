...
public class GlowController : MonoBehaviour
{
    ...
    [Range(0.1f, 3f)]
    [SerializeField] float blurScale = 1;
    ...
    public static void RegisterObject(GlowObjectCmd glowObj)
    {
        if (Instance != null)
        {
            Instance._glowableObjects.Add(glowObj);
        }
    }
    ...
    private void RebuildCommandBuffer()
    {
        ...
        for (int i = 0; i < _glowableObjects.Count; i++)
        {
            ...
            {
                for (int k = 0; k < _glowableObjects[i].Renderers[j].GetComponent<MeshFilter>().sharedMesh.subMeshCount; k++)
                {
                    _commandBuffer.DrawRenderer(_glowableObjects[i].Renderers[j], _glowMat, k);
                }
            }
        }
        ...
        _blurTexelSize = new Vector2(1.5f / (Screen.width >> 1), 1.5f / (Screen.height >> 1)) * blurScale;
